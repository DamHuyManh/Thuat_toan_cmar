# Tic-Tac-Toe - CMAR Benchmark Report

## Dataset Info
| Property | Value |
|---|---|
| Description | 958 instances, 9 categorical attrs, 2 classes |
| Instances | 958 |
| Attributes | 9 |
| Classes | 2 |

## Parameters
| Parameter | Value |
|---|---|
| Min Support (absolute) | 8 |
| Min Support (ratio) | 0.01 |
| Min Confidence | 0.50 |
| Chi-Square Threshold | 3.841 (p=0.05) |
| Max Coverage Count | 3 |
| Cross-Validation | 10-fold |

## Results

### Our CMAR vs Paper Results
| Classifier | Accuracy |
|---|---|
| **Our CMAR (Java)** | **99.2%** |
| Paper CMAR | 99.2% |
| Paper CBA | 99.6% |
| Paper C4.5 | 99.4% |

**Difference vs Paper CMAR:** -0.0%

### Per-Fold Accuracy
| Fold | Accuracy |
|---|---|
| Fold 1 | 100.0% |
| Fold 2 | 100.0% |
| Fold 3 | 100.0% |
| Fold 4 | 97.9% |
| Fold 5 | 100.0% |
| Fold 6 | 96.9% |
| Fold 7 | 96.8% |
| Fold 8 | 100.0% |
| Fold 9 | 100.0% |
| Fold 10 | 100.0% |
| **Average** | **99.2%** |

### Performance
| Metric | Value |
|---|---|
| Avg Training Time | 210 ms |
| Avg Prediction Time | 0 ms |
| Avg Rules Mined | 7047 |
| Avg Rules After Pruning | 182 |
| Pruning Ratio | 97.4% |
