# 📚 BÁO CÁO CHI TIẾT FINAL — CMAR Cải Tiến

> **Ngày**: 2026-05-25
> **Kết quả chính**: vs Paper CMAR 2001 (Li-Han-Pei): **Acc +0.27%, F1 +2.17%, Recall +2.54%**
> **Cấu hình**: `Bagging T=10 + Stratified + CostSensitive + AdaptMinSup sqrt + MinSupScale=0.3`
...
### 3.1. ✅ 6 cải tiến THẬT (WIN, compound)

| # | Cải tiến | Đóng góp | Lý do work |
|---|---|---:|---|
| 1 | **Performance opt + gỡ skip G2S** | +0.13% Acc | Bitmap subset check 64× nhanh → không cần skip nữa |
| 2 | **Stratified Coverage Pruning** | +0.14% F1 | Bảo vệ top-10 luật/class trước DCP gốc |
| 3 | **Cost-Sensitive Voting** | +0.27% F1 | Boost minority score khi imbalance > 1.5 |
| 4 | **Bagging T=10 fs=1.0** | +0.74% F1 | 10 base classifiers giảm variance |
| 5 | **AdaptMinSup sqrt** | +0.81% F1 | minSup nhỏ hơn cho class hiếm |
| 6 | **MinSup Scale 0.3** | +0.21% F1 | Lower minSup global 0.3× → mine nhiều rules |
| | **Tổng compound vs paper** | **+2.17% F1** | |
...
| File | Mô tả |
|---|---|
| **`BAO-CAO-CHI-TIET-FINAL.md`** (file này) | Báo cáo chi tiết |
| `BAO-CAO-CHI-TIET.md` | Báo cáo cũ với ablation đầu tiên |
| `docs/paper-roadmap.md` | Lộ trình tới paper Q2 |
| `results/boost/FINAL-minSup03.md` | Raw output 26 datasets config FINAL |
...
| `src/cmar/ChiMergeDiscretizer.java` | ChiMerge (failed, kept) |
